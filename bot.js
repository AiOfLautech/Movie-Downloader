...
bot.command("download", async (ctx) => {
  ...
  try {
    ...
    movies.forEach((movie, index) => {
      bot.action(`movie_${index}`, async (actionCtx) => {
        actionCtx.reply(`🎥 You selected: *${movie.title}*`, { parse_mode: "Markdown" });

        const qualityResponse = await axios.get(movie.link); // Adjust as per API response
        const qualities = qualityResponse.data.qualities || []; // Adjust key as per API response
        ...
        actionCtx.reply(
          `🎥 *Available Qualities for: ${movie.title}*\n\nSelect a quality to download directly:`,
          {
            parse_mode: "Markdown",
        ...
        // Handle quality selection
        qualities.forEach((quality, qIndex) => {
          bot.action(`quality_${index}_${qIndex}`, (qualityCtx) => {
            const directDownloadUrl = quality.download_url; // Replace with actual API key
            qualityCtx.reply(
              `📥 *Your download is ready:*\n\n🔗 [Download Link](${directDownloadUrl})`,
              { parse_mode: "Markdown" }
            );
